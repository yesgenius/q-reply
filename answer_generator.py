...
SHEET_LOG_PARAMS = "LOG_ANSWER_PARAMS"  # Sheet for model parameters
...
SIMILARITY_THRESHOLD = 0.8  # Threshold for both DB search and context filtering
...
class AnswerGenerator:
    ...
    def create_log_params_sheet(self, wb: Workbook) -> None:
        ...
        config_params = [
            ...
            (
                ...
                SIMILARITY_THRESHOLD,
                "Minimum similarity for DB search and Q&A context filtering",
            ),
            (
                "USE_CHAT_HISTORY",
                USE_CHAT_HISTORY,
                "Use Q&A pairs as chat history (True) or user context (False)",
            ),
            (
                "USE_CATEGORIES",
                USE_CATEGORIES,
                "Enable categorization and category-based filtering",
            ),
        ...
        # Add separator for category model parameters
        try:
    ...
    def create_output_file(self) -> tuple[Workbook, Path]:
        ...
        if LOG_ANSWER:
            ...
            # Add headers to LOG_ANSWER sheet
            headers = [
                "question",
                "category",
                "category_confidence",
    ...
    def search_similar_questions(
        ...
        Uses USE_CATEGORIES flag to determine if category filtering should be applied.

        Args:
            question: The question to search for.
            category: Optional category filter (used only if USE_CATEGORIES=True).
        ...
        try:
            ...
            # Simple logic: use category filter only if USE_CATEGORIES is True
            if USE_CATEGORIES and category:
                logger.debug(f"Searching with category filter: {category}")
                search_category = category
            else:
                logger.debug("Searching without category filter")
    ...
    def generate_answer_with_retry(
        ...
        Args:
            ...
            qa_pairs: Context Q&A pairs with similarity scores from search.

        Returns:
            Answer result dictionary or None on failure.
        """
        # Filter pairs by SIMILARITY_THRESHOLD before passing to LLM
        filtered_pairs = []
        ...
        logger.debug(
            f"Filtered {len(qa_pairs)} Q&A pairs to {len(filtered_pairs)} "
            f"using threshold {SIMILARITY_THRESHOLD}"
        )
        ...
        for attempt in range(1, MAX_RETRY_ATTEMPTS_ANSWER + 1):
            try:
                # Pass pre-filtered pairs to get_answer_prompt
                result_json, messages_list, raw_response = get_answer_prompt.run(
                    user_question=question,
                    qa_pairs=filtered_pairs,  # Using filtered pairs
                )
    ...
    def process_row(self, sheet_q: Worksheet, sheet_log: Worksheet | None, row_idx: int) -> bool:
        ...
        try:
            ...
            # Step 1: Categorize question only if USE_CATEGORIES is True
            category_result = None
            selected_category = None

            if USE_CATEGORIES and self.categories:
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Use Categories: {'ENABLED' if USE_CATEGORIES else 'DISABLED'}")
            logger.info(f"Similarity Threshold: {SIMILARITY_THRESHOLD}")
            logger.info(f"Context Mode: {'Chat History' if USE_CHAT_HISTORY else 'User Context'}")
            ...
            # Step 4: Load categories (only if USE_CATEGORIES is True)
            if USE_CATEGORIES:
                ...
                logger.info("Step 4: Skipping categories (USE_CATEGORIES=False)")
            ...
            get_answer_prompt.update_chat_history(USE_CHAT_HISTORY)  # Set context mode
            logger.info(
                f"Answer generation system initialized (context mode: "
                f"{'chat history' if USE_CHAT_HISTORY else 'user context'})"
            )
