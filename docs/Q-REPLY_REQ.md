...
2. **Запуск скрипта** `answer_generator.py`
   - **Инициализация:**
     ...
     - Проверяет наличие книги, листа и словаря на листе `QA.CATEGORY`; если книги и/или листа и/или словаря нет — выполняет поиск без использования категорий.
       - Если словарь есть, то проверяет заполненность всех категорий (колонка `A`) и всех ответов (колонка `B`) на листе `QA.CATEGORY`; если есть пропуск — завершение скрипта.
     - Открывает БД `QA`; если БД нет  — завершение скрипта.
     - Берёт словарь категорий из `Q_YYYY-MM-DD_HHMMSS.CATEGORY`.
     - Если текущий запуск — **новая обработка**:
   ...
   - **Обработка каждого вопроса из `Q_YYYY-MM-DD_HHMMSS.xlsx`:**
     ...
     - Генерация шаблона ответа на базе контекста с помощью `prompts.get_answer_prompt.run()`
   ...
   - **Логирование:**
     ...
     - Формирует лог на листе `Q_YYYY-MM-DD_HHMMSS.LOG` с информацией для контроля над процессом формирования ответа gigachat.client
...
│   ├── define_answer_prompt.py            # модуль промта для формирования ответа на вопрос с использованием динамического контекста, доработанная копия base_prompt.py
