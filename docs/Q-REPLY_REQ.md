...
### Контекст
  ...
  - **Решение:** Инструмент для формирования шаблона ответа на каждый вопрос, без учета текущей фактологии
...
#### Процесс выполнения category_filler.py
  ...
  2. **Запуск скрипта** `category_filler.py`
    - **Инициализация:**
      ...
      - Проверяет заполненность всех категорий (колонка `A`) и всех ответов (колонка `B`) на листе `QA.CATEGORY`; если есть пропуск — завершение скрипта.
      ...
      - **Обработка каждой строки из `QA_YYYY-MM-DD_HHMMSS.QA`**:
        - Если поле категории пустое или категория не входит в словарь `QA_YYYY-MM-DD_HHMMSS.CATEGORY`:
          ...
          - Формирует строку на листе `QA_YYYY-MM-DD_HHMMSS.LOG_CATEGORY` с информацией для контроля процесса формирования ответа (`gigachat.client`), если `log_category == True`:
            - положение каждой строки лога соответствует положению обрабатываемой строки на листе `QA_YYYY-MM-DD_HHMMSS.QA`;
            - колонки:
              - **Категории** — категории из `QA_YYYY-MM-DD_HHMMSS.QA`;
              - **Вопросы** — вопросы из `QA_YYYY-MM-DD_HHMMSS.QA`;
              - **category** — заполняется из JSON, который возвращает `define_category_prompt.run()`;
              - **confidence** — заполняется из JSON, который возвращает `define_category_prompt.run()`;
              - **reasoning** — заполняется из JSON, который возвращает `define_category_prompt.run()`;
              - **messages** — заполняется из `define_category_prompt.messages` после выполнения `define_category_prompt.run()`.
...
### UC2: Формирование векторной БД (DuckDB)

#### Входные данные
- Новый или обновленный `in/QA.xlsx`
...
2. **Запуск скрипта** `update_db.py`
   ...
   - **Инициализация:**
     ...
     - Копирует `in/QA.xlsx` в `out/QA_YYYY-MM-DD_HHMMSS.xlsx`
     - Берет словарь категорий из `QA_YYYY-MM-DD_HHMMSS.CATEGORY`
	 - Если текущий запуск - это новая обработка и если лист `QA_YYYY-MM-DD_HHMMSS.LOG_DB` существует, то его очистить или удалить+создать
     ...
     - Для каждого вопроса:
       - **Поиск вопроса в БД:**
         ...
         - **Вопрос не найден:**
           ...
           - Определяет категорию вопроса и ответа из словарь категорий с помощью gigachat.client 
           - Сохранение в БД: вопрос+эмбеддинг, ответ+эмбеддинг, категория
	     - Формирует строку на листе `QA_YYYY-MM-DD_HHMMSS.LOG_DB` с информацией для контроля над процессом формирования ответа gigachat.client
...
1. **Подготовка данных**
   ...
   - Определение категорий исторических вопросов в `QA.xlsx`
...
2. **Запуск скрипта** `answer_generator.py`
   - **Инициализация:**
	 ...
	 - Получает описание конференции из `Q_YYYY-MM-DD_HHMMSS.T.A2`
	 - Если текущий запуск - это новая обработка и если лист `Q_YYYY-MM-DD_HHMMSS.LOG_ANSWER` существует, то его очистить или удалить+создать
   
   - **Обработка каждого вопроса из `Q_YYYY-MM-DD_HHMMSS.xlsx`:**
     - Получает эмбеддинги вопрос с помощью embeddings.base_embedding
     - Определяет категорию из словаря категорий с помощью gigachat.client 
     - Фильтрация вопросов БД по определенной категории
     - Поиск N наиболее близких вопросов по cosine similarity
     - Формирование контекста для LLM:
       - из описания конференции
       - из N найденных вопросов-ответов
     - Генерация шаблона ответа базе контекста с помощью gigachat.client
     - Формирует строку на листе `Q_YYYY-MM-DD_HHMMSS.LOG_ANSWER` с информацией для контроля над процессом формирования ответа gigachat.client
     - Запоминает обработанную строку из `Q_YYYY-MM-DD_HHMMSS.Q`, чтобы при повторном старте после сбоя gigachat.client или embeddings.base_embedding начать со следующей

   - **Формирование результата:**
     - Таблица с колонками:
       - Исходный вопрос
       - Шаблон ответа
       - 2N колонок с N парами найденных вопросов-ответов
   
   - **Логирование:**
     - Формирование понятного лога для неподготовленного пользователя
	 - Формирует лог на листе `QA_YYYY-MM-DD_HHMMSS.LOG` с информацией для контроля над процессом формирования ответа gigachat.client

3. **Проверка результатов**
   - Пользователь проверяет качество ответов:
     - по результату на листе `Q_YYYY-MM-DD_HHMMSS.ANSWER`
...
- `out/QA_YYYY-MM-DD_HHMMSS.xlsx` 
  ...
  - Лист `Q_YYYY-MM-DD_HHMMSS.ANSWER` с шаблонами ответов
- Лог выполнения в терминале
...
│       ├── Лист "QA"
│       │   ├── A: Категоря (необязательное поле)
│       │   ├── B: Вопрос
│       │   └── C: Ответ
│       └── Лист "CATEGORY"
│           ├── A: Категоря
...
│   ├── some_prompt-3.py                   # модуль кастомного промта, доработанная копия base_prompt.py
│   └── ...
│
...
- **Тестирование:** Все модули должны содержать код полного теста в секции:
  ```python
...
- Использование готовых модулей без изменений
- Централизованная конфигурация через `.env` файл
- Подробное логирование для отслеживания процесса выполнения


## 📝 ПРИМЕЧАНИЯ
- Логи должны быть понятны пользователям без технической подготовки
- Система должна корректно обрабатывать кириллицу в Excel файлах
- При работе с API необходимо учитывать лимиты и реализовать retry-логику
